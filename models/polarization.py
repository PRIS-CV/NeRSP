...
def cues_from_stokes_stack_np(stokes_np):
    ...
    return cues
def colorize_cues_mine(pol_cues):
    import torch
    ...
    aolp_color = aolp_cmap(aolp.numpy().mean(-1))[...,:3]
    color_pol_cues['aolp'] = torch.Tensor(aolp_color, device=device)

    dolp = pol_cues['dolp']
    dolp_cmap = plt.get_cmap('GnBu')
    dolp_color = dolp_cmap(dolp.numpy().mean(-1))[...,:3]
    color_pol_cues['dop'] = torch.Tensor(dolp_color, device=device)
...
def calculate_pol_cues(stokes):
    import torch
    # stokes 1 512 3 3
    s0 = stokes[...,0]
    s1 = stokes[...,1]
    s2 = stokes[...,2]
    dolp_r = torch.clip(torch.sqrt(s1[...,0]**2+s2[...,0]**2)/(s0[...,0]+1e-10),0,1)  # 1 512
    aolp_r = torch.atan2(s2[...,0],s1[...,0])/2
    dolp_g = torch.clip(torch.sqrt(s1[...,1]**2+s2[...,1]**2)/(s0[...,1]+1e-10),0,1)  # 1 512
    aolp_g = torch.atan2(s2[...,1],s1[...,1])/2
    dolp_b = torch.clip(torch.sqrt(s1[...,2]**2+s2[...,2]**2)/(s0[...,2]+1e-10),0,1)  # 1 512
    aolp_b = torch.atan2(s2[...,2],s1[...,2])/2

    cues_0_r = dolp_r * torch.cos(2*aolp_r)
    cues_1_r = dolp_r * torch.sin(2*aolp_r)
    
    cues_0_g = dolp_g * torch.cos(2*aolp_g)
    cues_1_g = dolp_g * torch.sin(2*aolp_g)

    
    cues_0_b = dolp_b * torch.cos(2*aolp_b)
    cues_1_b = dolp_b * torch.sin(2*aolp_b)

    I_unpol = s0
    I_pol_cues_0 = torch.cat((cues_0_r[...,None],cues_0_g[...,None],cues_0_b[...,None]),-1)
    I_pol_cues_1 = torch.cat((cues_1_r[...,None],cues_1_g[...,None],cues_1_b[...,None]),-1)
    
    return I_unpol,I_pol_cues_0,I_pol_cues_1 # 1 512 3
