...
class MVASLoss(nn.Module):
    ...
    def get_rgb_loss(self,rgb_values, rgb_gt,surface_mask):
        # print(rgb_gt.max(),rgb_gt.min())
        # print(rgb_values.max(),rgb_values.min())
        rgb_values = rgb_values[surface_mask]
        rgb_gt = rgb_gt[surface_mask]
    ...
    def forward(self, model_outputs, model_inputs):
        ...
        rgb_gt = model_inputs['img']
        rgb_output = model_outputs['rgb']
        surface_mask = model_outputs['surface_mask']
        loss_img = self.get_rgb_loss(rgb_output,rgb_gt.cuda(),surface_mask)
